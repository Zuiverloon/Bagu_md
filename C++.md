...
### 多态 动态绑定/静态绑定(static/dynamic binding)
...
控制块:负责管理与资源生命周期相关的元数据，创建一个 sharedptr 就会创建一个控制块，共享同一资源的sharedptr有同一个控制块，拷贝 sharedptr 就让强引用+1，销毁一个就-1，强引用变 0 时，调用删除器释放资源，但是控制块仍然存在直到弱引用也变为 0
...
协程是可暂停继续执行函数。如果一个函数有co_await,co_yield,co_return，他就是协程。编译器看到co_await,co_yield,co_return后会改写成协程对象+状态机，通过看返回类型promise_type知道返回什么对象、挂起策略、怎么保存结果
协程被挂起，相当于函数返回，只保存一些寄存器但不发生线程切换，还是原来的线程，只不过执行其他代码去了。
协程创建时发生了什么？

1. 分配coroutine frame
2. 构造promise_type
3. 调用get_return_object()
4. 调用initial_suspend() 是否立即执行，取决于initial_suspend()，如果是suspend_never就立即执行，如果是suspend_always就等resume

promise_type:只需要实现协程里面使用到的函数
get_return_object()创建协程函数的返回值，必须要
initial_suspend()必须要
final_suspend()必须要，协程结束时调用
unhandled_exception()必须要，协程内部异常进入这里
若使用co_return需要return_void(没有返回值的话), return_value(有返回值的话)
若使用co_yield，需要yield_value()
co_await 暂停执行，直到被我或其他runtime调用h.resume()
co_yield
co_return

协程业务逻辑不同，编译器生成的代码路径也不同，但是骨架是固定的，都是把函数改写成状态机，根据我的函数填充状态机内容
...
1. allocator先看线程缓存和全局空闲链表(Bins)有没有足够空间，有的话直接分配，不然要切换到内核态申请内存

**free比malloc慢**

free还需要整理合并(coalesce)内存碎片
...
值传递:复制实参的值，生成一个新的局部变量  
引用传递:传递的是原变量的地址  
指针传递:本质上是值传递，变量是指针类型，所以可以改变原数据
...
## 浅拷贝shallow copy vs 深拷贝deep copy
...
## 移动赋值运算符 copy assignment operator
...
预处理(preprocessing):处理#开头的指令，生成.i / .ii 文件，包含展开后的完整源代码
编译(Compilation):将预处理后的代码转换为汇编代码.s 文件（词法分析 - 语法分析 - 语义分析 - 构建抽象语法树 - 优化代码）
汇编(Assembly):将汇编代码转换为机器码，将.s 文件转为 .o/obj 目标文件，包含符号表，但不能直接运行，可能引用外部符号如库函数
链接(Linking):将多个目标文件和库文件合并为可执行文件，符号解析，重定位，静态链接，动态链接，生成.exe(win) / 可执行文件(linux)
...
st.push_back(2);
st.push_back(3);
auto it = upper_bound(st.begin(), st.end(), 2);
--it;
cout << *it << "\n";
```
...
const cast:去掉 const 修饰
reinterpret cast:指针类型的转换，指针整数类型的转换（不安全）
...
memory_order_acquire:获取，不允许它之后的读写被重排到它以前  
memory_order_release:释放，不允许它之前到读写重拍到它之后  
memory_order_acq_rel:同时具备 ac 和 rel  
memory_order_seq_cst:最强，所有线程看到的顺序一致

**happens before**
如果操作A happens before B，那么

1. 可见性：A 对内存的所有修改（写变量），B 都能看到。
2. 有序性：A 在逻辑上先于 B 执行，编译器和 CPU 不会破坏这种因果顺序。

**建立happens before的场景**

1. 同线程内：语句A写在语句B之前
2. mutex：A unlock 后 -> B lock
3. 原子变量: release -> acquire
4. 线程启动: std::thread t(func) -> func() 内部
5. func() 结束 -> t.join() 返回后
...
void solve(int cas)
{
	thread th1(printNumber);
	thread th2(printAlpha);
	th1.join();
	th2.join();
}
```
...
见contest_template/live_coding/sort.cpp
...
SPSC 单消费者单生产者
MPMC 多消费者多生产者
见contest_template/live_coding/LockFreeQueue.cpp
