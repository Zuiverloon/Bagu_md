...
**为什么进入内核开销较大**
切权限，换栈，破坏分支预测，TLB受影响，一次系统调用大概100-1000ns
**为什么context switching 开销更大？**
内核态是同一个进程、线程内部切换，地址空间不变、缓存TLB页表等都有效。cs需要保存更多寄存器状态，决定下一个被调度的线程，如果线程不属于同一个进程还要改地址空间、切换页表、TLB失效，切换后会cache code start(许多cache miss)

## TLB

在cpu内部很小的空间，存虚拟地址-物理地址，存最近访问过的地址，
